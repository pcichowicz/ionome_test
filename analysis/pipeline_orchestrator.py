...
from analysis.stages.StatPlots import VolcanoPlotStage, PLSDAStage, XICPlottingStage, HeatmapClusteringStage
...
def run_pipeline_for_study(study_id: str,
    ...
    if config.dataset_profile == "cohort_with_qc":
        # cohort_with_qc has no dedicated standard-mixture injection to run real SST
        # against (see project notes) — system_suitability is a documented no-op here.
        # standards_only runs the real SystemSuitabilityStage and must not be stubbed.
        context.qc_metrics["system_suitability"] = {"skipped": "not applicable for cohort_with_qc — no standard-mixture injection in dataset"}
